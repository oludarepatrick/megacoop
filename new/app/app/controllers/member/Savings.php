...
class Savings extends BASE_Controller {
    ...
    public function index() {
        ...
        if ($this->form_validation->run()) {
            ...
            $this->data['savings'] = $this->info->get_savings(['users.id'=> $this->user->id,'users.coop_id' => $this->coop->id], 1000, true);
        }
        $this->data['filter_total_savings'] = $this->common->sum_this('savings', $where, 'amount');
        $this->data['total_savings'] = $this->common->sum_this('savings', ['user_id'=> $this->user->id,'tranx_type'=>'credit','savings.coop_id' => $this->coop->id], 'amount');
        $this->data['year'] = $where['savings.year'];
        $this->data['title'] = lang('savings');
        $this->data['controller'] = lang('savings');
        $this->layout->set_app_page_member('member/savings/index', $this->data);
    }
    ...
    public function add() {
        ...
        if ($this->form_validation->run()) {
            ...
            $this->data['controller'] = lang('savings');
            $this->layout->set_app_page_member('member/savings/add', $this->data);
        }
    }

    public function preview($id) {
        $id = $this->utility->un_mask($id);
        $this->data['savings'] = $this->info->get_savings_details(['savings.user_id'=> $this->user->id,'savings.id' => $id, 'users.coop_id' => $this->coop->id]);
        $this->data['title'] = lang('print');
        $this->data['controller'] = lang('savings');
        $this->layout->set_app_page_member('member/savings/preview', $this->data);
    }
