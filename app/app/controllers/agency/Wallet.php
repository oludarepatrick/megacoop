...
class Wallet extends BASE_Controller {
    ...
    public function index() {
        ...
        $where = [
            'agent_wallet.user_id' => $this->user->id,
        ...
        if ($this->form_validation->run()) {
            ...
            $this->data['wallet'] = $this->info->get_agent_wallet($where, FALSE, true);
            $where['agent_wallet.status'] = 'successful';
            ...
            $this->data['wallet'] = $this->info->get_agent_wallet($where, 1000, true);
            $where['agent_wallet.status'] = 'successful';
            $where['tranx_type'] = 'credit';
            $this->data['total_wallet_credit'] = $this->common->sum_this('agent_wallet', $where, 'amount');
            $where['tranx_type'] = 'debit';
        ...
        $this->layout->set_app_page_agency('wallet/index', $this->data);
    }
    ...
    public function load_wallet()
    {
        ...
        if ($this->form_validation->run()) {
            ...
            $tranx_ref = strtoupper(str_replace(' ', '', $this->user->first_name . $this->user->last_name)) . date('dYmHis') . 'WALLET';
            ...
            if ($gate_way == 1) {
                $pay_data = [
                    ...
                    'reference' => $tranx_ref,
                    'bearer' => 'account',
                    'callback_url' => base_url() . 'agency/wallet/verify',
                ];
            } elseif ($gate_way == 2) {
                $pay_data = [
                    'email' => ($this->user->email) ? $this->user->email : 'noemail@noemail.com',
                    ...
                    'currency' => 'NGN',
                    'tx_ref' => $tranx_ref,
                    'redirect_url' => base_url() . 'agency/wallet/verify_flutter',
            ...
            $this->common->add('agent_wallet', $wallet_data);
            if ($gate_way == 1) {
            ...
            $this->data['controller'] = lang('wallet');
            $this->layout->set_app_page_agency('wallet/load_wallet', $this->data);
        }
    }
    ...
    public function verify()
    {
        ...
        if ($verify) {
            if ($verify['status'] == true and $verify['data']['status'] == 'success') {
                ...
                if ($this->common->get_this('agent_wallet', $where)) {
                    $this->common->update_this('agent_wallet', $where, ['status' => 'successful']);
    ...
    public function verify_flutter()
    {
        ...
        if ($verify) {
            if ($verify['status'] == 'success') {
                ...
                if ($this->common->get_this('agent_wallet', $where)) {
                    $this->common->update_this('agent_wallet', $where, ['status' => 'successful', 'amount' => $verify['data']['amount_settled']]);
                    $this->session->set_flashdata('message', lang('wallet_loading_successful'));
                } else {
                    $this->session->set_flashdata('error', lang('act_unsuccessful'));
                }
