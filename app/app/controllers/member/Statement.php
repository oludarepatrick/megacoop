...
class Statement extends BASE_Controller {
    ...
    public function index(){
        ...
        if ($this->form_validation->run()) {
            ...
            $this->layout->set_app_page_member('member/statement/index', $this->data);
        }
    }
    
    public function loan_statement($post){
        $this->data['user'] = $this->common->get_this('users',['id'=>$this->user->id]);
        $this->data['loan_type'] = $this->common->get_this('loan_types',['id'=>$post['loan_type']]);
        $this->data['active_loan'] = $this->common->get_this('loans',['loan_type_id'=>$post['loan_type'], 'status'=>'disbursed', 'user_id' => $this->user->id]);
        ...
        $this->data['controller'] = lang('statement');
        $this->layout->set_app_page_member('member/statement/loan', $this->data);
...
//        var_dump($this->data['savings']);exit;
        ...
        $this->data['controller'] = lang('statement');
        $this->layout->set_app_page_member('member/statement/savings', $this->data);
    }
